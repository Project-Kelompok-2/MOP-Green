    ...
    if($num != 0){
      if($logEmail==$email && $password==$pass){
        if(!isset($_SESSION['samepage'])){
          ...
          echo json_encode(['nama_depan' => $namaDepan, 'nama_belakang' => $namaBelakang, 'asal_institusi' => $asalInstitusi, 'kegiatan' => $kegiatan, 'email' => $logEmail, 'username' => $username, 'passsword' => $password, 'level' => $lvl]);
          die;
        }
